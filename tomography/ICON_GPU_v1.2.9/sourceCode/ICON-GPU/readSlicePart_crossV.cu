...
void readSlicePart_crossV(cunfftplan1d plan,CUFFTDATATYPE *fhat,CUFFTDATATYPE *bfCU,FILE *infile,MrcHeader *inhead,int yNum,int omitindex)
{
    int i,k,j;
    float *slice;
	...
	cunfft_trafo_1d(plan,bfCU+j*inhead->nx,fhat);
	j++;
    }
